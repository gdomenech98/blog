...
### 1. Environment Check and Pull Changes
...
- Task non-completed at the end of your workday:
...
<aside>
⚠️ Before implementing a feature, I **have to ensure that I am in the "green" state**, meaning it is functional. 
...
<aside>
🚨 **Never implement a feature if I am not in the "green" state because then, how can I be sure that what I am implementing works?**
...
1. **Red:** In this phase, a developer writes a failing test that defines a small unit of functionality. The test is written before any code is implemented, and it initially fails because there is no code to satisfy it.
2. **Green:** In this phase, the developer writes the minimum amount of code required to make the failing test pass. The goal is to implement the functionality needed to satisfy the test, without concern for code quality or optimization at this stage.
3. **Refactor:** After the test passes, the developer can refactor the code to improve its structure, readability, and efficiency. Refactoring is about making the code better without changing its behavior. This step helps maintain a clean and maintainable codebase.
