...
## Principles
- **Service Independence/ Unique responsibility:** **Each microservice operates independently** and **can be developed, deployed, and scaled separately** from the others. This independence allows teams to work on different microservices without affecting the entire system.
...
- **API-Based Communication:** Microservices communicate with each other through APIs, following clear contracts (eg Open API) and versioning. Typically using lightweight protocols like HTTP/REST or gRPC. This allows them to be language-agnostic and enables interoperability.
...
- **Fault Tolerance**: Assume that all can fail. Design the system to handle failures gracefully.
- **Observability**: Monitor the system to detect and understand failures and predict upcoming issues. Logs, métrics, distributed traces, alerts, latencies, bottle necks, etc.
- **Backward Compatibility**: Ensure that changes in one microservice do not break others.
...
## What to do and what to don't do

- Share DB across multiple microservices
...
- No Backward compatibility between changes.
- Omnipotent orchestrator --> produce bottle necks.
- Excessive synchronous inter-service communication. Chain synchronous processes as REST calls between services cause **coupling** and **latency (bottle necks)**. In case of event driven microservices, **do not ignore eventual consistency** and **do not force ACID transactions** between microservices. 
- Do **not plan or assume that it can fail**.